...
async def execute_complete_jira_extraction(
    ...
    try:
        ...
        logger.info(f"Γ£à Step 1 completed: {projects_result['projects_count']} projects, {projects_result['issue_types_count']} issue types")
        ...
        logger.info(f"Γ£à Step 2 completed: {statuses_result['statuses_count']} statuses, {statuses_result['project_relationships_count']} relationships")
        ...
        logger.info("=" * 80)
        logger.info(f"≡ƒöì DEBUG: Job status should be RUNNING during Step 3")



        try:
            logger.info(f"≡ƒöì DEBUG: About to call _extract_issues_with_changelogs_for_complete_job")
            ...
            logger.info(f"Γ£à Step 3 completed: {issues_result}")
            logger.info(f"≡ƒöì DEBUG: Returned from _extract_issues_with_changelogs_for_complete_job - job should still be RUNNING")
        except Exception as issues_error:
            logger.error(f"Γ¥î Step 3 (Issues extraction) failed: {issues_error}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise


        logger.info(f"≡ƒöì DEBUG: Step 3 progress updated to 100% - job complete (dev status runs in background)")
        ...
        logger.info(f"≡ƒöì DEBUG: All 3 steps complete - about to call reset_job_countdown to set job to FINISHED")
        await reset_job_countdown(tenant_id, job_id, "Complete Jira extraction finished successfully", job_start_time)
        logger.info(f"≡ƒöì DEBUG: reset_job_countdown completed - job should now be FINISHED then READY")

        logger.info("=" * 100)
        logger.info("Γ£à COMPLETE JIRA EXTRACTION FINISHED SUCCESSFULLY")
...
async def reset_job_countdown(tenant_id: int, job_id: int, message: str, job_start_time=None):
    ...
    try:
        ...
        # Γ£à FIX: Calculate next_run from when job STARTED, not when it FINISHED
        ...
        if last_run_started_at:
            ...
            logger.info(f"ΓÅ░ Calculating next_run from job START time (last_run_started_at)")
        else:
            # Fallback: if no start time, use current time
            next_run = now + timedelta(minutes=schedule_interval_minutes)
            logger.info(f"ΓÅ░ Calculating next_run from current time (no last_run_started_at)")

        logger.info(f"ΓÅ░ TIMEZONE CHECK: Current time (now) = {now}")
        logger.info(f"ΓÅ░ TIMEZONE CHECK: Job start time (last_run_started_at) = {last_run_started_at}")
        logger.info(f"ΓÅ░ TIMEZONE CHECK: Job sync time (sync_time) = {sync_time}")
        logger.info(f"ΓÅ░ TIMEZONE CHECK: Next run calculated = {next_run}")
        logger.info(f"ΓÅ░ TIMEZONE CHECK: Schedule interval = {schedule_interval_minutes} minutes")
        ...
        logger.info(f"Γ£à Jira job marked as FINISHED")
        ...
        logger.info(f"Γ£à Jira job set to READY - next run scheduled for {next_run}")
...
async def _extract_projects_and_issue_types(
    ...
    if job_id:
        ...
        success = queue_manager.publish_transform_job(
            ...
            data_type='jira_projects_and_issue_types',  # ≡ƒöº Use correct step name for UI
            job_id=job_id,
            provider=provider_name,
            last_sync_date=job_start_time.isoformat(),
            first_item=True,  # Γ£à Single message (first)
            last_item=True,   # Γ£à Single message (last)
            token=token  # ≡ƒöæ Include token in message
        )

        if success:
            logger.info(f"Γ£à Published projects and issue types to transform queue with first_item=true")
        else:
            logger.error(f"Γ¥î Failed to publish projects and issue types to transform queue")
...
async def _extract_statuses_and_relationships(
    ...
    logger.info(f"≡ƒöì [DEBUG] Starting _extract_statuses_and_relationships for tenant {tenant_id}, integration {integration_id}")
    ...
    with database.get_read_session_context() as db:
        ...
        if not projects_result:
            logger.error(f"≡ƒöì [DEBUG] No projects found in database for tenant {tenant_id}, integration {integration_id}")
        ...
        logger.info(f"≡ƒöì [DEBUG] Found {len(project_keys)} projects for status extraction: {project_keys}")
    ...
    for i, project_key in enumerate(project_keys):
        try:
            ...
            logger.info(f"≡ƒöì [DEBUG] Calling get_project_statuses for project {project_key}")
            project_statuses_response = jira_client.get_project_statuses(project_key)
            logger.info(f"≡ƒöì [DEBUG] get_project_statuses returned: {len(project_statuses_response) if project_statuses_response else 0} issue types for project {project_key}")
            ...
            if project_statuses_response:
                ...
                logger.info(f"≡ƒöì [DEBUG] About to queue transform job for project {project_key}, raw_data_id={raw_data_id}")
                ...
                queued = queue_manager.publish_transform_job(
                    ...
                    integration_id=integration_id,
                    raw_data_id=raw_data_id,
                    data_type='jira_statuses_and_relationships',  # ≡ƒöº Use correct step name for UI
                    job_id=job_id,
                    provider='jira',
                ...
                logger.info(f"≡ƒöì [DEBUG] Transform job queued: {queued} for project {project_key}")
            ...
            logger.error(f"Γ¥î [DEBUG] Exception in project loop for {project_key}: {e}")
            import traceback
    ...
    logger.info(f"≡ƒöì [DEBUG] Getting unique statuses count from database for tenant {tenant_id}, integration {integration_id}")
    from app.core.database import get_database
    ...
    with database.get_read_session_context() as session:
        ...
        logger.info(f"≡ƒöì [DEBUG] Found {unique_statuses_count} unique statuses in database")



    logger.info(f"≡ƒöì [DEBUG] _extract_statuses_and_relationships completed successfully - returning success=True")
...
async def _extract_issues_with_changelogs_for_complete_job(
    ...
    while True:
        ...
        if not issues:
            if batches_stored == 0:
                logger.warning(f"Step 3: ΓÜá∩╕Å No issues found matching JQL query: {jql}")
            else:
                logger.info(f"Step 3: Γ£à Reached end of results - fetched {total_issues_processed:,} total issues in {batches_stored} batches")
            break
        ...
        batches_stored += 1
        logger.info(f"Step 3: Γ£à Stored {len(raw_data_ids)} individual issues from batch #{batches_stored} in {insert_time:.0f}ms")
        ...
        for raw_data_id in raw_data_ids:
            success = queue_manager.publish_transform_job(
                ...
                integration_id=integration_id,
                raw_data_id=raw_data_id,
                data_type='jira_issues_with_changelogs',  # ≡ƒöº Use correct step name for UI
                provider='jira',
                token=token  # ≡ƒöæ Include token in message
            )
        ...
        if failed_count > 0:
            logger.warning(f"Step 3: ΓÜá∩╕Å Published {published_count}/{len(raw_data_ids)} issues to queue in {publish_time:.0f}ms ({failed_count} failed)")
        else:
            logger.info(f"Step 3: Γ£à Published {published_count} issues to transform queue in {publish_time:.0f}ms")
        ...
        if batches_stored % 10 == 0:
            logger.info(f"Step 3: ≡ƒôè Progress update - {batches_stored} batches complete ({total_issues_processed:,} issues)")

        # Check if this is the last page
        if is_last or not next_page_token:
            logger.info(f"Step 3: Γ£à Reached last page - fetched {total_issues_processed:,} total issues in {batches_stored} batches")
            break

    # Final completion message
    if batches_stored > 0:
        logger.info(f"Step 3: Γ£à COMPLETED - Stored and queued {batches_stored} batches ({total_issues_processed:,} total issues)")
    else:
        logger.info(f"Step 3: Γä╣∩╕Å No issues found to extract")
...
async def _extract_dev_status_for_complete_job(
    ...
    if not issue_keys:
        logger.info("Step 4: Γä╣∩╕Å No issues with code changes found - skipping dev status extraction")
...
async def _extract_issues_with_changelogs(
    ...
    # ≡ƒÄ» HANDLE CASE: No issues found - send completion message to transform
    if not all_issues:
        ...
        success = queue_manager.publish_transform_job(
            tenant_id=tenant_id,
            integration_id=integration_id,
            raw_data_id=None,  # ≡ƒöº None signals completion message
            ...
            first_item=True,
            last_item=True,
            last_job_item=True,  # ≡ƒÄ» Complete the job since no data to process
            token=token  # ≡ƒöæ Include token in message
        )

        if success:
            logger.info(f"Γ£à Sent completion message to transform queue (no issues found)")
            published_count = 1
        else:
            logger.error(f"Γ¥î Failed to send completion message to transform queue")
    ...
    for i, issue in enumerate(all_issues):
        ...
        # ≡ƒÄ» CRITICAL: Issues should NEVER complete the job when there are development issues
        ...
        success = queue_manager.publish_transform_job(
            ...
            first_item=first_item,
            last_item=last_item,
            last_job_item=last_job_item,  # ≡ƒÄ» Complete job if no dev_status needed
            token=token  # ≡ƒöæ Include token in message
        )
        ...
        if success:
            published_count += 1
            if last_job_item:
                logger.info(f"Γ£à Issue {issue.get('key')} queued with last_job_item=True (will complete job)")
            else:
                logger.debug(f"≡ƒô¥ Issue {issue.get('key')} queued (first={first_item}, last={last_item})")
    ...
    # ≡ƒÄ» NEW ARCHITECTURE: Queue dev_status extractions for issues with development field
    ...
    if issues_with_code_changes:
        ...
        for i, issue_key in enumerate(issues_with_code_changes):
            ...
            last_job_item = last_item  # ≡ƒÄ» Mark the final dev_status as the job completion trigger
            ...
            success = queue_manager.publish_extraction_job(
                ...
                token=token  # ≡ƒöæ Forward token to dev_status extraction
            )

            if success:
                dev_status_queued += 1
                logger.debug(f"Γ£à Queued dev_status extraction for {issue_key} (first={first_item}, last={last_item}, job_end={last_job_item})")
            else:
                logger.error(f"Γ¥î Failed to queue dev_status extraction for {issue_key}")
...
async def extract_projects_and_issue_types_endpoint(
    ...
    try:
        ...
        logger.info(f"≡ƒôï Adding background task: execute_complete_jira_extraction(integration_id={integration_id}, tenant_id={tenant_id}, job_id={job_id})")
        ...
        logger.info(f"Γ£à Background task added successfully")

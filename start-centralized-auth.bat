...
if %errorlevel% == 0 (
    echo ⚠️  Port %1 is already in use. Skipping %2.
    goto :eof
) else (
    echo ✅ Port %1 is free. Starting %2...
)
goto :eof
...
if %errorlevel% == 0 goto skip_auth
echo Starting Auth Service on port 4000...
cd services\auth-service
start "Auth Service" cmd /k "python -m uvicorn app.main:app --host 0.0.0.0 --port 4000 --reload"
cd ..\..
...
if %errorlevel% == 0 goto skip_backend
...
if %errorlevel% == 0 goto skip_etl
echo Starting ETL Service on port 8000...
cd services\etl-service
start "ETL Service" cmd /k "python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload"
cd ..\..
timeout /t 3 /nobreak >nul
:skip_etl

...
const WitsHierarchiesPage: React.FC<WitsHierarchiesPageProps> = ({ embedded = false }) => {
  ...
  // Queue all hierarchies for vectorization
  const handleQueueForVectorization = async () => {
    try {
      ...
      showSuccess('Queued for Vectorization', `${result.queued_count} hierarchies queued for vectorization`)
    } catch (err) {
      console.error('Error queueing vectorization:', err)
      showError('Queue Failed', 'Failed to queue hierarchies for vectorization')
    } finally {
      setQueueing(false)
    }
  }

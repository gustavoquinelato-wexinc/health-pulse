...
def paginate_reviews(session: Session, graphql_client: GitHubGraphQLClient, pr_node_id: str,
    ...
    try:
        ...
        while cursor:
            # Check rate limit
            if graphql_client.should_stop_for_rate_limit():
                logger.warning("⚠️ Rate limit threshold reached during review pagination")
                return {
                    'success': False,
                    'error': 'Rate limit threshold reached',
                    'last_cursor': cursor
...
def paginate_comments(session: Session, graphql_client: GitHubGraphQLClient, pr_node_id: str,
    ...
    try:
        ...
        while cursor:
            # Check rate limit
            if graphql_client.should_stop_for_rate_limit():
                logger.warning("⚠️ Rate limit threshold reached during comment pagination")
                return {
                    'success': False,
                    'error': 'Rate limit threshold reached',
                    'last_cursor': cursor
...
def paginate_review_threads(session: Session, graphql_client: GitHubGraphQLClient, pr_node_id: str,
    ...
    try:
        ...
        while cursor:
            # Check rate limit
            if graphql_client.should_stop_for_rate_limit():
                logger.warning("⚠️ Rate limit threshold reached during review thread pagination")
                return {
                    'success': False,
                    'error': 'Rate limit threshold reached',
                    'last_cursor': cursor

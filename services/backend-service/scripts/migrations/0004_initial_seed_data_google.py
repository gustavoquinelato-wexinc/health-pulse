...
def apply(connection):
    ...
    try:
        ...
        # 2b. Create default worker configuration for Google tenant
...
def rollback(connection):
    ...
    try:
        print("🔄 Rolling back Migration 0004: Initial Seed Data Google")

        # Delete seed data in reverse order of creation, handling foreign key constraints properly
        print("📋 Removing ETL jobs...")
        cursor.execute("DELETE FROM etl_jobs WHERE tenant_id = (SELECT id FROM tenants WHERE name = 'Google');")

        print("📋 Removing user permissions...")
        cursor.execute("DELETE FROM users_permissions WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")

        print("📋 Removing user sessions...")
        cursor.execute("DELETE FROM users_sessions WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")
        ...
        cursor.execute("DELETE FROM statuses_mappings WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")
        ...
        cursor.execute("DELETE FROM custom_fields_mapping WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")

        print("📋 Removing custom fields...")
        cursor.execute("DELETE FROM custom_fields WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")
        ...
        connection.commit()
        print(f"SUCCESS: Migration 0004 rolled back successfully")

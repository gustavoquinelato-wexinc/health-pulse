...
def apply(connection):
    ...
    try:
        ...
        if ai_gateway_base_url and ai_gateway_api_key and ai_model:
            ...
            if ai_fallback_model and ai_fallback_model != ai_model:
                cursor.execute("""
                    ...
                    RETURNING id;
                """, (
                    "apple_ai_gateway_fallback", "ai_provider", None, encrypted_ai_key, ai_gateway_base_url,
        ...
        cursor.execute("""
            ...
            RETURNING id;
        """, (
            "wex_fabric", "data_warehouse", "apple_fabric_user", None, "https://fabric.wex.com",
            None, None,
        ...
        cursor.execute("""
            ...
            RETURNING id;
        """, (
            "active_directory", "identity_provider", "apple_ad_user", None, "https://login.microsoftonline.com",
            None, None,
...
def rollback(connection):
    ...
    try:
        ...
        cursor.execute("DELETE FROM job_schedules WHERE client_id = (SELECT id FROM clients WHERE name = 'Apple');")

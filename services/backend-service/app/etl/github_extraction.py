...
async def extract_github_repositories(
    ...
    token: str = None  # ≡ƒöæ Job execution token
) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"≡ƒÜÇ [GITHUB] Starting repository extraction for tenant {tenant_id}, integration {integration_id}")
    ...
    try:
        ...
        with get_read_session_context() as db:  # This is a standalone function, not a method
            ...
            # ≡ƒöæ IMPORTANT: Capture current date at extraction start
            ...
            logger.info(f"≡ƒôà Search date range: {start_date} to {end_date}")
            logger.info(f"≡ƒöì Repository filters: {name_filters}")
            logger.info(f"≡ƒÅó Organization: {org}")
            logger.info(f"≡ƒöä Next sync will use new_last_sync_date: {end_date}")
            ...
            logger.info(f"Γ£à Exiting database session, integration_data prepared")
    except Exception as e:
        logger.error(f"Γ¥î Error in database session: {e}")
        import traceback
    ...
    try:
        ...
        logger.info(f"≡ƒôè Retrieved {len(all_repositories)} total repositories from GitHub API")

        # Process each repository as a separate raw_extraction_data record
        if all_repositories:
            # ≡ƒöæ LOOP 1: Extract all repos and queue to transform
            logger.info(f"≡ƒôï [LOOP 1] Queuing {len(all_repositories)} repositories to transform")
            for i, repo in enumerate(all_repositories):
                ...
                # ≡ƒöæ Transform worker expects 'repositories' key (array), so wrap single repo in array
                raw_data_id = store_raw_extraction_data(
                    integration_id, tenant_id, "github_repositories",
                    {
                        'repositories': [repo],  # ≡ƒöæ Wrap in array with 'repositories' key
                        'search_date_range': {
                            'start_date': integration_data['start_date'],
                            'end_date': integration_data['end_date']
                        },
                ...
                # ≡ƒöæ first_item=true ONLY on first repository
                # ≡ƒöæ last_item=true ONLY on last repository
                # ≡ƒöæ last_repo=true ONLY on last repository (signals to Step 2 that this is the final repo)
                ...
                success = queue_manager.publish_transform_job(
                    ...
                    job_id=job_id,
                    provider='github',
                    last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
                    new_last_sync_date=end_date,  # ≡ƒöæ Used for job completion (extraction end date)
                    token=token,  # ≡ƒöæ Include token in message
                    first_item=is_first,              # ≡ƒöæ True only on first repo
                    last_item=is_last,                # ≡ƒöæ True only on last repo
                    last_job_item=False,              # ≡ƒöæ Never set to True here - job continues to PR extraction
                    last_repo=is_last                 # ≡ƒöæ True only on last repo - signals to Step 2
                )

                if not success:
                    logger.error(f"Failed to queue repository {i+1}/{len(all_repositories)} for transform")

            logger.info(f"Γ£à [LOOP 1 COMPLETE] All {len(all_repositories)} repositories queued to transform")

            # ≡ƒöæ LOOP 2: Queue each repository to Step 2 extraction (NO database query)
            logger.info(f"≡ƒôï [LOOP 2] Queuing {len(all_repositories)} repositories to Step 2 extraction")
            for i, repo in enumerate(all_repositories):
                is_first = (i == 0)
                is_last = (i == len(all_repositories) - 1)

                # Queue PR extraction for this repository
                # ≡ƒöæ NO database query - use repo data directly from extraction
                ...
                success_pr = queue_manager.publish_extraction_job(
                    ...
                    extraction_data={
                        ...
                        'integration_id': integration_id  # ≡ƒöæ Pass integration_id to avoid DB query
                    },
                    job_id=job_id,
                    provider='github',
                    last_sync_date=last_sync_date,  # ≡ƒöæ Pass for incremental sync
                    new_last_sync_date=end_date,
                    token=token,  # ≡ƒöæ Include token in message
                    first_item=is_first,              # ≡ƒöæ True only on first repo
                    last_item=is_last,                # ≡ƒöæ True only on last repo
                    last_job_item=False,              # ≡ƒöæ Never set to True here - job continues
                    last_repo=is_last                 # ≡ƒöæ True only on last repo - signals to Step 2
                )
            ...
            logger.info(f"Γ£à [LOOP 2 COMPLETE] All {len(all_repositories)} repositories queued to Step 2 extraction")
            total_repositories = len(all_repositories)

        logger.info(f"Γ£à [GITHUB] Repository extraction completed: {total_repositories} repositories found and queued for transform and PR extraction")

        # ≡ƒÄ» Handle case when NO repositories were found
        if total_repositories == 0:
            logger.info(f"≡ƒôñ No repositories found - sending completion message to transform queue")
            # ≡ƒöæ Completion message pattern:
            ...
            success = queue_manager.publish_transform_job(
                tenant_id=tenant_id,
                integration_id=integration_id,
                raw_data_id=None,  # ≡ƒöæ Completion message marker
                data_type='github_repositories',
                job_id=job_id,
                provider='github',
                last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
                ...
                first_item=True,
                last_item=True,
                last_job_item=True,  # ≡ƒöæ Signal job completion - transform will forward to embedding
                token=token  # ≡ƒöæ Include token in message
            )
            if not success:
        ...
        return {
            ...
            'last_sync_date': last_sync_date,  # ≡ƒöæ Pass to PR extraction
            'message': f'Successfully extracted and queued {total_repositories} repositories'
        }

    except Exception as e:
        logger.error(f"Γ¥î [GITHUB] Error extracting repositories: {e}")
        import traceback
        logger.error(f"Full traceback: {traceback.format_exc()}")
...
def store_raw_extraction_data(
    ...
    try:
        ...
        with database.get_write_session_context() as db:
            ...
            logger.info(f"Γ£à Stored raw GitHub data (type={data_type}) with ID {raw_data_id}")
            return raw_data_id

    except Exception as e:
        logger.error(f"Γ¥î Error storing raw GitHub data: {e}")
        import traceback
        logger.error(f"Full traceback: {traceback.format_exc()}")
        return None
...
def _search_github_repositories_paginated(
    ...
    logger.info(f"≡ƒöì [GITHUB SEARCH] Starting repository search with token={token[:20]}..., org={org}, filters={filters_to_use}")
    ...
    try:
        ...
        logger.info(f"≡ƒöì [GITHUB SEARCH] Base query: {base_query}")
        ...
        if filters_to_use:
            for filter_pattern in filters_to_use:
                ...
                logger.info(f"≡ƒöì [GITHUB SEARCH] Added filter pattern: {filter_pattern} in:name")

        # Add specific repo names
        if additional_repo_names:
            logger.info(f"≡ƒöì [GITHUB SEARCH] Adding {len(additional_repo_names)} additional repo names")
        ...
        if not search_patterns:
            logger.warning("≡ƒöì [GITHUB SEARCH] No search patterns provided for GitHub repository search")
            return []

        logger.info(f"≡ƒöì [GITHUB SEARCH] Total search patterns: {len(search_patterns)}")
        ...
        logger.info(f"≡ƒöì [GITHUB SEARCH] Executing {len(pattern_batches)} search requests to stay within character limit")

        # Accumulate ALL repositories from all pages and patterns
        all_repositories = []

        # Execute each batch
        logger.info(f"≡ƒöì [GITHUB SEARCH] Creating HTTP client...")
        with httpx.Client() as client:
            logger.info(f"≡ƒöì [GITHUB SEARCH] HTTP client created successfully")
            for i, batch_patterns in enumerate(pattern_batches, 1):
                ...
                logger.info(f"≡ƒöì [GITHUB SEARCH] Batch {i}/{len(pattern_batches)}: Query length = {len(full_query)}, Query = {full_query[:100]}...")
                ...
                logger.info(f"≡ƒöì [GITHUB SEARCH] Making HTTP request to {endpoint}?q={urllib.parse.quote(full_query)}")
                try:
                    ...
                    while True:
                        if next_url:
                            ...
                            logger.debug(f"≡ƒöì [GITHUB SEARCH] Using next URL from Link header: {next_url[:100]}...")
                            response = client.get(next_url, headers=headers, timeout=30.0)
                        else:
                            # First request with params
                            current_params = {**params, "page": page}
                            logger.debug(f"≡ƒöì [GITHUB SEARCH] Sending request with params: {current_params}")
                            response = client.get(endpoint, params=current_params, headers=headers, timeout=30.0)

                        logger.info(f"≡ƒöì [GITHUB SEARCH] Response status: {response.status_code}")

                        # Check for rate limit (403 or 429)
                        if response.status_code in (403, 429):
                            logger.warning(f"ΓÅ╕∩╕Å Rate limit hit during repository search: {response.status_code}")
                        ...
                        if not items:
                            logger.info(f"≡ƒöì [GITHUB SEARCH] No more items in response")
                            break
                        ...
                        if len(accumulated_repos) >= 1000:
                            logger.info(f"≡ƒöì [GITHUB SEARCH] Reached 1000 result limit")
                            is_last_page = True
                        ...
                        if link_header:
                            ...
                            for link in link_header.split(','):
                                if 'rel="next"' in link:
                                    ...
                                    logger.debug(f"≡ƒöì [GITHUB SEARCH] Found next URL in Link header")
                                    break

                        if not next_url:
                            logger.info(f"≡ƒöì [GITHUB SEARCH] No next link in header, pagination complete for this pattern batch")
                            is_last_page = True

                        # Accumulate all repos (don't yield yet)
                        if is_last_page:
                            logger.info(f"≡ƒöì [GITHUB SEARCH] End of pagination for pattern batch {i}/{len(pattern_batches)}: accumulated {len(accumulated_repos)} repos")
                    ...
                    logger.warning(f"ΓÅ╕∩╕Å Rate limit hit during repository search batch {i}/{len(pattern_batches)}")
        ...
        logger.info(f"≡ƒöì [GITHUB SEARCH] Total repositories found: {len(all_repositories)}")
        return all_repositories

    except Exception as e:
        logger.error(f"Γ¥î Error searching GitHub repositories: {e}")
        import traceback
        logger.error(f"Full traceback: {traceback.format_exc()}")
        return []
...
async def github_extraction_worker(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        integration_id = message.get('integration_id')  # ≡ƒöæ Extract from message
        job_id = message.get('job_id')
        repository_id = message.get('repository_id')

        logger.info(f"≡ƒöÇ [ROUTER] Extraction worker received message for repo {repository_id}")

        # ≡ƒöæ Extract token from message
        ...
        if message.get('type') == 'github_nested_extraction_recovery':
            ...
            logger.info(f"≡ƒöä [ROUTER] Routing to extract_nested_recovery (PR {message.get('pr_id')})")
            result = await extract_nested_recovery(
                tenant_id=tenant_id,
                integration_id=integration_id,  # ≡ƒöæ For service-to-service auth
                job_id=job_id,
                pr_id=message.get('pr_id'),
                pr_node_id=message.get('pr_node_id'),
                nested_nodes_status=message.get('nested_nodes_status', {}),
                owner=message.get('owner'),  # ≡ƒöæ Pass repo info from message
        ...
        # ROUTER: Check if this is nested data continuation
            ...
            logger.info(f"≡ƒöÇ [ROUTER] Routing to extract_nested_pagination (nested_type={message.get('nested_type')})")
            result = await extract_nested_pagination(
                ...
                new_last_sync_date=message.get('new_last_sync_date'),  # ≡ƒöæ Forward from message
                nested_index=message.get('nested_index', 0),  # ≡ƒöæ Forward nested type info
                total_nested_types=message.get('total_nested_types', 1),  # ≡ƒöæ Forward nested type info
                is_last_nested_type=message.get('is_last_nested_type', False),  # ≡ƒöæ Forward nested type info
                last_item=message.get('last_item', False),  # ≡ƒöæ Forward flag from message
                last_job_item=message.get('last_job_item', False),  # ≡ƒöæ Forward flag from message
                last_repo=message.get('last_repo', False),  # ≡ƒöæ Forward flag from message
                last_pr=message.get('last_pr', False),  # ≡ƒöæ Forward flag from message
                token=token  # ≡ƒöæ Include token in message
            )
        else:
            # FRESH OR NEXT PR PAGE
            is_fresh = message.get('pr_cursor') is None
            logger.info(f"≡ƒöÇ [ROUTER] Routing to extract_github_prs_commits_reviews_comments ({'fresh' if is_fresh else 'next'} page)")
            result = await extract_github_prs_commits_reviews_comments(
                ...
                first_item=message.get('first_item', False),  # ≡ƒöæ Forward from message
                last_item=message.get('last_item', False),  # Forward from message
                last_sync_date=message.get('last_sync_date'),  # ≡ƒöæ Old sync date for filtering
                new_last_sync_date=message.get('new_last_sync_date'),  # ≡ƒöæ New sync date for job completion
                token=token,  # ≡ƒöæ Include token in message
                last_repo=message.get('last_repo', False),  # ≡ƒöæ Forward from message
                last_pr=message.get('last_pr', False)  # ≡ƒöæ Forward from message
            )

        return result

    except Exception as e:
        logger.error(f"Γ¥î Extraction worker error: {e}")
        import traceback
...
async def extract_github_prs_commits_reviews_comments(
    ...
    first_item: bool = False,  # ≡ƒöæ NEW: Received from message
    last_item: bool = False,
    last_sync_date: Optional[str] = None,  # ≡ƒöæ Old sync date for filtering
    new_last_sync_date: Optional[str] = None,  # ≡ƒöæ New sync date for job completion
    last_repo: bool = False,  # ≡ƒöæ NEW: True if this is the last repository
    last_pr: bool = False,     # ≡ƒöæ NEW: True if this is the last PR of the last repository
    token: str = None  # ≡ƒöæ Job execution token
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"≡ƒÜÇ Starting GitHub PR extraction - {'Fresh' if is_fresh else 'Next'} page for {owner}/{repo_name}")

        # ≡ƒöæ IMPORTANT: Capture current date at start of extraction
        ...
        # ≡ƒöæ owner and repo_name should ALWAYS be provided from message (no DB query for data)
        ...
        logger.info(f"Γ£à Using owner and repo_name from message: {owner}/{repo_name}")
        logger.info(f"≡ƒôà Using last_sync_date for filtering: {last_sync_date}")

        # ≡ƒöæ Get integration (service-to-service, not data processing)
        ...
        logger.info(f"≡ƒôÑ Fetching PR page for {owner}/{repo_name} (cursor: {pr_cursor or 'None'})")
        try:
            pr_page = await github_client.get_pull_requests_with_details(
                owner, repo_name, pr_cursor
            )
        except GitHubRateLimitException as e:
            logger.warning(f"ΓÅ╕∩╕Å Rate limit hit during PR extraction: {e}")
            _save_rate_limit_checkpoint(
                job_id=job_id,
                tenant_id=tenant_id,
            ...
            # ≡ƒöæ Send completion message to transform queue to signal job completion
            # This allows the job to finish gracefully even though we hit rate limit
            logger.info(f"≡ƒôñ Sending completion message to transform queue due to rate limit")
            queue_manager.publish_transform_job(
                ...
                job_id=job_id,
                provider='github',
                last_sync_date=last_sync_date,  # ≡ƒöæ Forward last_sync_date
                first_item=False,
                last_item=True,
                last_job_item=True  # ≡ƒöæ Signal job completion
            )
        ...
        if not prs:
            ...
            # ≡ƒöæ If this is the last repository (last_repo=true), send completion message
            if last_repo:
                logger.info(f"≡ƒÄ» [COMPLETION] No PRs found and this is the last repo - sending completion message")
                queue_manager.publish_transform_job(
                    tenant_id=tenant_id,
                    integration_id=integration.id,
                    raw_data_id=None,  # ≡ƒöæ Completion message marker
                    ...
                    provider='github',
                    last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
                    new_last_sync_date=extraction_end_date,  # ≡ƒöæ Used for job completion (extraction end date)
                    first_item=False,
                    last_item=True,
                    last_job_item=True,  # ≡ƒöæ Signal job completion
                    last_repo=last_repo,
                ...
                logger.info(f"Γ£à Completion message queued to transform")

            return {'success': True, 'prs_processed': 0}

        logger.info(f"≡ƒôª Processing {len(prs)} PRs from page")
        ...
        # ≡ƒöæ If no last_sync_date provided, use 2-year default (same as repository extraction)
        if not last_sync_date:
            # Using module-level imports from line 24
            two_years_ago = datetime.now() - timedelta(days=730)
            last_sync_date = two_years_ago.strftime('%Y-%m-%d')
            logger.info(f"≡ƒôà No last_sync_date provided, using 2-year default: {last_sync_date}")

        logger.info(f"≡ƒöì Filtering PRs by last_sync_date: {last_sync_date}")
            ...
            for pr in prs:
                try:
                    ...
                    if pr_updated_dt > last_sync_dt:
                        ...
                        logger.info(f"≡ƒ¢æ PR {pr.get('number')} updated at {pr_updated_at_str} is older than last_sync_date {last_sync_date}, stopping pagination")
        ...
        logger.info(f"Γ£à Filtered {len(filtered_prs)} PRs (from {len(prs)} total)")

        # STEP 2: Split PRs into individual raw_data entries
        # ≡ƒöæ Clean structure: pr_data contains all nested data, no duplication
        raw_data_ids = []
        with get_write_session_context() as db:
            for pr in filtered_prs:
                raw_data = {
                    'pr_id': pr['id'],
                    'owner': owner,  # ≡ƒöæ Include owner for transform to lookup repository
                    'repo_name': repo_name,  # ≡ƒöæ Include repo_name for transform to lookup repository
                    'full_name': f"{owner}/{repo_name}",  # ≡ƒöæ Include full_name for easier analysis
                    'pr_data': pr  # ≡ƒöæ pr_data already contains commits, reviews, comments, reviewThreads
                }
                raw_data_id = _store_raw_extraction_data(
                    db, tenant_id, integration_id,
                    'github_prs_commits_reviews_comments',
                    raw_data, pr['id']  # ≡ƒöæ Use PR external_id (GitHub PR node_id)
                )
                raw_data_ids.append(raw_data_id)

        logger.info(f"≡ƒÆ╛ Stored {len(raw_data_ids)} raw data entries")
        ...
        # ≡ƒöæ This determines if last_item should be true on the last PR
        ...
        logger.info(f"≡ƒöì Has nested pagination: {has_nested_pagination}")

        # STEP 4: Queue all PRs to transform
        # ≡ƒöæ first_item=true ONLY on first PR when received from message
        # ≡ƒöæ last_item=true ONLY on last PR when no more PR pages to fetch
        # ≡ƒöæ last_job_item=true ONLY on last PR when:
        ...
        for i, raw_data_id in enumerate(raw_data_ids):
            is_first = (i == 0 and first_item)  # ≡ƒöæ Use received first_item flag, not is_fresh
            is_last_pr_in_page = (i == len(raw_data_ids) - 1 and not has_next_page)  # Last only if no more PR pages

            # ≡ƒöæ Set last_item=true ONLY when:
            ...
            # ≡ƒöæ NOTE: last_item signals end of THIS repository's PR extraction AND job completion
            is_last_item = (is_last_pr_in_page and not has_nested_pagination and last_repo and last_pr)

            # ≡ƒöæ Only set last_job_item=true if:
            # - This is the last PR in the page AND no more PR pages
            # - AND this is the last repo (last_repo=true)
            # - AND this is the last PR of the last repo (last_pr=true)
            ...
            queue_manager.publish_transform_job(
                ...
                new_last_sync_date=extraction_end_date,  # ≡ƒöæ Used for job completion (extraction end date)
                first_item=is_first,
                last_item=is_last_item,  # ≡ƒöæ True only when all conditions met
                last_job_item=is_last_job_item,  # ≡ƒöæ True only when all conditions met
                last_repo=last_repo,
                last_pr=last_pr,
                token=token  # ≡ƒöæ Include token in message
            )

        logger.info(f"≡ƒôñ Queued {len(raw_data_ids)} PRs to transform")

        # STEP 5: Loop through each PR and queue nested pagination if needed
        # ≡ƒöæ Build list of nested types that need pagination for this PR
        for pr in filtered_prs:
            pr_node_id = pr['id']
            ...
            # ≡ƒöæ RELAY FLAGS: Pass last_pr=true to LAST nested type
            ...
            for nested_index, (nested_type, nested_cursor) in enumerate(nested_types_needing_pagination):
                ...
                queue_manager.publish_extraction_job(
                    tenant_id=tenant_id,
                    integration_id=integration_id,  # ≡ƒöæ Use integration_id from function parameter
                    extraction_type='github_prs_commits_reviews_comments',  # Route to github_extraction_worker
                    extraction_data={
                        'owner': owner,  # ≡ƒöæ Pass repo info instead of repository_id
                        'repo_name': repo_name,
                        'full_name': f"{owner}/{repo_name}",
                        'pr_node_id': pr_node_id,
                        'nested_type': nested_type,
                        'nested_cursor': nested_cursor,
                        'nested_index': nested_index,                    # ≡ƒöæ Index of this nested type
                        'total_nested_types': total_nested_types,        # ≡ƒöæ Total nested types for this PR
                        'is_last_nested_type': is_last_nested_type       # ≡ƒöæ Is this the last nested type?
                    },
                    job_id=job_id,
                    provider='github',
                    last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
                    new_last_sync_date=extraction_end_date,  # ≡ƒöæ Used for job completion (extraction end date)
                    first_item=False,
                    last_item=False,                    # ≡ƒöæ Will be set to true only on final nested page
                    last_job_item=False,                # ≡ƒöæ Will be set to true only on final nested page
                    last_repo=last_repo,                # ≡ƒöæ Forward: true if last repository
                    last_pr=is_last_nested_type         # ≡ƒöæ True only on last nested type
                )
                logger.info(f"≡ƒôñ Queued {nested_type} pagination (index {nested_index}/{total_nested_types}, is_last_nested_type={is_last_nested_type}, last_pr={is_last_nested_type})")

        logger.info(f"≡ƒôñ Queued nested pagination messages for PRs with incomplete data")

        # STEP 5: Queue next PR page if exists (and we didn't hit early termination)
        # ≡ƒöæ RELAY FLAGS: Pass last_repo=true to next page
        ...
        if has_next_page and not early_termination:
            ...
            queue_manager.publish_extraction_job(
                ...
                new_last_sync_date=extraction_end_date,  # ≡ƒöæ Used for job completion (extraction end date)
                first_item=False,
                last_item=False,           # ≡ƒöæ Not the last item yet - more PRs to process
                last_job_item=False,       # ≡ƒöæ Not job completion yet
                last_repo=last_repo,       # ≡ƒöæ Forward: true if last repository
                last_pr=last_pr            # ≡ƒöæ Forward: true if last PR of last repo
            )
            logger.info(f"≡ƒôñ Queued next PR page to extraction queue with last_repo={last_repo}, last_pr={last_pr}")
        elif early_termination:
            logger.info(f"≡ƒ¢æ Early termination due to old PRs, not queuing next page")
        ...
        # ≡ƒöæ NOTE: Completion message is ONLY sent when there are NO PRs to extract
        ...
        logger.info(f"Γ£à PR extraction completed: {len(prs)} PRs processed")
        return {
            'success': True,
        ...
        logger.error(f"Γ¥î Error in GitHub PR extraction: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return {'success': False, 'error': str(e)}
...
async def extract_nested_pagination(
    ...
    nested_type: str,
    nested_cursor: str,
    owner: Optional[str] = None,  # ≡ƒöæ Repository owner
    repo_name: Optional[str] = None,  # ≡ƒöæ Repository name
    full_name: Optional[str] = None,  # ≡ƒöæ Full repository name
    last_sync_date: Optional[str] = None,
    new_last_sync_date: Optional[str] = None,  # ≡ƒöæ NEW: Extraction end date for job completion
    nested_index: int = 0,
    total_nested_types: int = 1,
    is_last_nested_type: bool = False,
    last_item: bool = False,  # ≡ƒöæ NEW: Received from message
    last_job_item: bool = False,  # ≡ƒöæ NEW: Received from message
    last_repo: bool = False,  # ≡ƒöæ NEW: True if this is the last repository
    last_pr: bool = False,     # ≡ƒöæ NEW: True if this is the last PR of the last repository
    token: str = None  # ≡ƒöæ Job execution token
) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"≡ƒÜÇ Extracting nested {nested_type} for PR {pr_node_id}")

        from app.core.database_router import get_read_session_context, get_write_session_context
        from app.core.config import AppConfig
        from app.etl.queue.queue_manager import QueueManager

        # ≡ƒöæ Use owner/repo_name from message (no DB query for data)
        if not owner or not repo_name:
        ...
        # ≡ƒöæ Get integration and GitHub token (service-to-service, not data processing)
        with get_read_session_context() as db:
            integration = db.query(Integration).filter(
                Integration.id == integration_id,
                Integration.tenant_id == tenant_id
        ...
        logger.info(f"≡ƒôÑ Fetching {nested_type} page for PR {pr_node_id}")
        ...
        try:
            ...
            logger.warning(f"ΓÅ╕∩╕Å Rate limit hit during {nested_type} extraction: {e}")
            _save_rate_limit_checkpoint(
                job_id=job_id,
                tenant_id=tenant_id,
            ...
            # ≡ƒöæ Send completion message to transform queue to signal job completion
            # This allows the job to finish gracefully even though we hit rate limit
            logger.info(f"≡ƒôñ Sending completion message to transform queue due to rate limit in {nested_type}")
            queue_manager.publish_transform_job(
                tenant_id=tenant_id,
                integration_id=integration.id,
                raw_data_id=None,  # ≡ƒöæ Completion message marker
        ...
        # ≡ƒöæ Include repo info for transform to lookup repository
        raw_data = {
            'pr_id': pr_node_id,
            'owner': owner,  # ≡ƒöæ Include repo info for transform
            'repo_name': repo_name,
        ...
        with get_write_session_context() as db:
            raw_data_id = _store_raw_extraction_data(
                db, tenant_id, integration_id,
                'github_prs_nested',  # ≡ƒöæ Renamed from github_prs_commits_reviews_comments
                raw_data, pr_node_id  # ≡ƒöæ Use PR node_id as external_id
            )

        logger.info(f"≡ƒÆ╛ Stored nested {nested_type} data (has_more={has_more})")

        # STEP 3: Determine if this is the last item to queue
        # ≡ƒöæ last_item=true ONLY if:
        ...
        #    - AND last_pr=true (this is the last PR of the last repo)
        # ≡ƒöæ NOTE: last_item signals "end of step" - only true on FINAL item of ENTIRE job
        is_last_item = (is_last_nested_type and not has_more and last_repo and last_pr)

        # ≡ƒöæ last_job_item=true ONLY if:
        #    - This is the last nested type (is_last_nested_type=true)
        #    - AND there are no more pages for this nested type (not has_more)
        #    - AND last_repo=true (this is the last repository)
        #    - AND last_pr=true (this is the last PR of the last repo)
        # ≡ƒöæ NOTE: last_job_item signals "end of job" - same conditions as last_item for nested extraction
        ...
        queue_manager.publish_transform_job(
            ...
            provider='github',
            last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
            new_last_sync_date=new_last_sync_date,  # ≡ƒöæ Used for job completion (extraction end date)
            first_item=False,
            last_item=is_last_item,  # ≡ƒöæ True only on last page of last nested type of last PR of last repo
            last_job_item=is_last_job_item,  # ≡ƒöæ True only on last page of last nested type of last PR of last repo
            last_repo=last_repo,
            last_pr=last_pr,
            token=token  # ≡ƒöæ Include token in message
        )

        logger.info(f"≡ƒôñ Queued {nested_type} page to transform (last_item={is_last_item}, last_job_item={is_last_job_item})")

        # STEP 5: If more pages exist, queue next nested page to extraction
        # ≡ƒöæ RELAY FLAGS: Pass last_repo and last_pr to next nested page
        if has_more:
            queue_manager.publish_extraction_job(
                tenant_id=tenant_id,
                integration_id=integration_id,
                ...
                extraction_data={
                    ...
                    'nested_index': nested_index,                    # ≡ƒöæ Forward nested type info
                    'total_nested_types': total_nested_types,        # ≡ƒöæ Forward nested type info
                    'is_last_nested_type': is_last_nested_type       # ≡ƒöæ Forward nested type info
                },
                job_id=job_id,
                provider='github',
                last_sync_date=last_sync_date,  # ≡ƒöæ Used for filtering (old_last_sync_date)
                ...
                last_repo=last_repo,            # ≡ƒöæ Forward: true if last repository
                last_pr=last_pr,                # ≡ƒöæ Forward: true if last PR of last repo
                token=token  # ≡ƒöæ CRITICAL: Forward token to nested extraction
            )
            logger.info(f"≡ƒôñ Queued next {nested_type} page to extraction queue with last_repo={last_repo}, last_pr={last_pr}")
        # ≡ƒöæ NOTE: Do NOT send completion message here!
        ...
        logger.info(f"Γ£à Nested {nested_type} extraction completed (items: {len(nested_data['nodes'])})")
        return {
            'success': True,
            'nested_type': nested_type,
        ...
        logger.error(f"Γ¥î Error in nested pagination for {nested_type}: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return {'success': False, 'error': str(e)}
...
async def extract_nested_recovery(
    ...
    """
    try:
        logger.info(f"≡ƒöä Resuming nested extraction for PR {pr_id} from checkpoint")
        ...
        for nested_type in nested_types:
            ...
            if node_status.get('fetched') and not node_status.get('has_next_page'):
                ...
                logger.info(f"ΓÅ¡∩╕Å  Skipping {nested_type} (already complete)")
                continue

            if node_status.get('fetched') and node_status.get('has_next_page'):
                # Continue pagination from saved cursor
                logger.info(f"≡ƒôÑ Continuing {nested_type} pagination from cursor")
                ...
                result = await extract_nested_pagination(
                    ...
                    owner=owner,  # ≡ƒöæ Pass repo info
                    repo_name=repo_name,
                    full_name=full_name,
                ...
                # Fetch from scratch
                logger.info(f"≡ƒôÑ Fetching {nested_type} from scratch")
        ...
        logger.info(f"Γ£à Nested extraction recovery completed for PR {pr_id}")
        return {'success': True, 'pr_id': pr_id}

    except Exception as e:
        logger.error(f"Γ¥î Error in nested extraction recovery: {e}")
        import traceback
        logger.error(f"Traceback: {traceback.format_exc()}")
        return {'success': False, 'error': str(e)}
...
def _save_checkpoint(
    ...
    try:
        ...
        with database.get_write_session_context() as db:
            ...
            logger.info(f"Γ£à Saved checkpoint: {checkpoint_data}")
    except Exception as e:
...
def _save_rate_limit_checkpoint(
    ...
    try:
        ...
        with database.get_write_session_context() as db:
            ...
            logger.info(f"Γ£à Saved rate limit checkpoint: node_type={rate_limit_node_type}, reset_at={checkpoint_data['rate_limit_reset_at']}")
    except Exception as e:
...
def _set_job_start_time(job_id: int, tenant_id: int):
    ...
    try:
        ...
        with database.get_write_session_context() as db:
            ...
            logger.info(f"Γ£à Set last_run_started_at to {job_start_time} for job {job_id}")
    except Exception as e:
